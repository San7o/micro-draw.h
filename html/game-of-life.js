...
function findWasmBinary() {
    return locateFile('game-of-life.wasm');
}
...
var ASM_CONSTS = {
  69472: () => { Module.canvas.focus(); },  
 69497: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 69732: () => { window.addEventListener("keydown", (event) => { var key = stringToNewUTF8(event.key.value); var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); Module._RGFW_handleKeyEvent(key, code, 1); _free(key); _free(code); }, true); window.addEventListener("keyup", (event) => { var key = stringToNewUTF8(event.key.value); var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); Module._RGFW_handleKeyEvent(key, code, 0); _free(key); _free(code); }, true); },  
 70690: () => { var canvas = document.getElementById('canvas'); canvas.addEventListener('drop', function(e) { e.preventDefault(); if (e.dataTransfer.file < 0) return; var filenamesArray = []; var count = e.dataTransfer.files.length; var drop_dir = '.rgfw_dropped_files'; Module._RGFW_mkdir(drop_dir); for (var i = 0; i < count; i++) { var file = e.dataTransfer.files[i]; var path = '/' + drop_dir + '/' + file.name.replace("//", '_'); var reader = new FileReader(); reader.onloadend = (e) => { if (reader.readyState != 2) { out('failed to read dropped file: '+file.name+': '+reader.error); } else { var data = e.target.result; Module._RGFW_writeFile(path, new Uint8Array(data), file.size); } }; reader.readAsArrayBuffer(file); var filename = stringToNewUTF8(path); filenamesArray.push(filename); Module._RGFW_makeSetValue(i, filename); } Module._Emscripten_onDrop(count); for (var i = 0; i < count; ++i) { _free(filenamesArray[i]); } }, true); canvas.addEventListener('dragover', function(e) { e.preventDefault(); return false; }, true); },  
 71716: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 71787: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 71844: () => { return window.mouseX || 0; },  
 71875: () => { return window.mouseY || 0; },  
 71906: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 72053: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 72106: () => { return window.innerWidth; },  
 72136: () => { return window.innerHeight; },  
 72167: () => { Module.requestFullscreen(false, true); },  
 72206: () => { Module.exitFullscreen(false, true); },  
 72242: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
...
function assignWasmExports(wasmExports) {
  ...
  Module['_RGFW_handleKeyEvent'] = _RGFW_handleKeyEvent = createExportWrapper('RGFW_handleKeyEvent', 3);
  Module['_RGFW_handleKeyMods'] = _RGFW_handleKeyMods = createExportWrapper('RGFW_handleKeyMods', 7);
  Module['_Emscripten_onDrop'] = _Emscripten_onDrop = createExportWrapper('Emscripten_onDrop', 1);
  Module['_RGFW_makeSetValue'] = _RGFW_makeSetValue = createExportWrapper('RGFW_makeSetValue', 2);
  Module['_RGFW_mkdir'] = _RGFW_mkdir = createExportWrapper('RGFW_mkdir', 1);
  Module['_RGFW_writeFile'] = _RGFW_writeFile = createExportWrapper('RGFW_writeFile', 3);
